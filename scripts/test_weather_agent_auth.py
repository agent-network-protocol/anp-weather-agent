    ...
    for city in cities:
        ...
        if status == 200:
            ...
            if isinstance(data, dict) and data.get("status") == "1":
                ...
                if forecasts:
                    logger.info(
                        f"Retrieved forecast data for {city}: {forecasts[0].get('province')} {forecasts[0].get('city')}"
                    )
                else:
...
async def test_nl_api(auth_client: DIDWbaAuthHeader) -> bool:
    ...
    questions = ["明天北京天气怎么样？", "上海本周会下雨吗？"]

    all_successful = True

    for question in questions:
        status, data = await send_request(
            url, auth_client, method="POST", json_data={"question": question}
        )
...
async def test_subscription_api(auth_client: DIDWbaAuthHeader) -> bool:
    ...
    status, data = await send_request(
        subscribe_url, auth_client, method="POST", json_data=subscription_data
    )
    ...
    if status == 200:
        ...
        if isinstance(data, dict) and not data.get("success", True):
            logger.info(
                f"Subscription API response: {data.get('msg', 'Unknown message')}"
            )
            # 虽然是白名单错误，但API正常工作
    ...
    if status == 200:
        ...
        if isinstance(data, dict) and not data.get("success", True):
            logger.info(
                f"Subscription status API response: {data.get('msg', 'Unknown message')}"
            )
            # 虽然是白名单错误，但API正常工作
            ...
            if isinstance(data, dict) and data.get("success"):
                ...
                logger.info(
                    f"Subscription status: {status_info.get('status', 'Unknown')}"
                )
                status_test_passed = True
            else:
