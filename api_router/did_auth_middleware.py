...
async def generate_did_auth_token(authorization: str, domain: str) -> str:
    ...
    try:
        ...
        payload = {
            "sub": did,
            "exp": current_time
            + timedelta(seconds=300),  # Token 5 minutes later expires
            "iat": current_time,
        }
...
async def authenticate_did_request(
    ...
    try:
        ...
        if not authorization:
            ...
            raise HTTPException(
                status_code=401,
                detail="Authorization header missing, you can use tool https://service.agent-network-protocol.com/anp-explorer/ to access the URL",
            )
